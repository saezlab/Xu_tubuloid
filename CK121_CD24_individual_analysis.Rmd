---
title: "Single-cell analysis on CD24+ sorted cells"
...
The cell communities are going to be defined using Louvain algorithm.
...
Thus first we rename the cell cluster, and discard those afterwards.

```{r rename_idents}
ren_id <- setNames(c(rep("NA", length(levels(Idents(SeuratObject))))),
            levels(Idents(SeuratObject)))
```

We identify *hematopoietic cells* using the cannonical marker of this lineage, that
is the PTPRC gene expression. This highlights clusters 0, 1, 3, 7 and 10 as 
hematoietic cells.

```{r PTPRC,fig.width=4, fig.height=4}
FeaturePlot(SeuratObject, reduction = "umap", features = "PTPRC", label = TRUE)

ren_id[c("0", "1", "3", "7", "10")] <- paste0("Hematopoietic","_",c("0", "1", "3", "7", "10"))
# ren_id[c("0", "1", "3", "7", "10")] <- "Hematopoietic"

```

Similarly, we identify potential *endothelial cells* using PECAM1.
```{r PECAM1,fig.width=4, fig.height=4}
FeaturePlot(SeuratObject, reduction = "umap", features = "PECAM1", label = TRUE)

ren_id[c("6")] <- paste0("Endothelial","_6")
```


Finally, we took a look at the markers from previous section for a rational 
assignment of clusters to cell types.


| Cluster 7 | exclude hematopoietic |
...
```{r cell_ann}
ren_id["2"] <- paste0("LOH, MTAL cell","_2")
ren_id["4"] <- paste0("STCs","_4")
ren_id["5"] <- paste0("PTI/STCs","_5")
ren_id["8"] <- paste0("IC-A","_8")
ren_id["9"] <- paste("PECs","_9")
...
Finally we remove the contaminant cell populations from the gene sorting that 
are not epithelial cells.

```{r assign_filter}
SeuratObject <- SeuratObject[, !grepl("^(Hematopoietic|Endothelial)", 
                                      Idents(SeuratObject))]
...
In conclusion, the new clustering outcome lead to similar results as before. Moreover,
a novel population is detached from LOH, the cluster 3, that it will be discovered 
later as DTC after studying cluster-specific biomarkers.
We kept this cell clusters as the final outcome of the cell clustering.
...
top_markers = apply(sg$specScore, 2, function(x) names(head(sort(x, decreasing = TRUE), n = 10)))
...
| Cluster | rational |
|:---|:---|
| Cluster 0 | LOH, TAL cells |
| Cluster 1 | STC, 77 AKAP STCs? 67% VCAM P   |
| Cluster 2 | STC, 58% AKAP |
| Cluster 3 | DCT, no AKAP12 |
| Cluster 4 | PECs 56% AKAP, 24% Vcam1 47 PECS? KLK6  |
| Cluster 5 | PT- cells, Slc34a1, 11% AKAP |
| Cluster 6 | Type A IC Slc4a1  |

```{r cell_assign2}
ren_id <- c("0"="LOH, TAL cells",
            "1"="STC_1",
            "2"="STC_2",
            "3"="DCT",
            "4"="PEC",
...
Finally, we create UMAPs showing the final cell assignment
...
```{r CK121_CD24_adhoc_dotplot_final, fig.path='./output/CK121_CD24/figs/', dev = c('png','pdf')}
DotPlot(SeuratObject, features = c("SLC12A1", "SLC12A3", "KLK6", "CLDN1", "SLC4A1", "AKAP12", "VCAM1", "SLC34A1"), dot.scale = 14) + 
        coord_flip() + scale_y_discrete(position = "right") +
        theme(axis.text.x = element_text(angle = 45, hjust = 0))
```

```{r CK121_CD24_context_dotplot_final, fig.width=7, fig.height=12, fig.path='./output/CK121_CD24/figs/', dev = c('png','pdf')}
context_genes <- getGmt("./data/Prior/Kramann_context.gmt")
print(DotPlot(SeuratObject, features = unlist(geneIds(context_genes)), dot.scale = 14) + 
        coord_flip() + scale_y_discrete(position = "right") +
        theme(axis.text.x = element_text(angle = 45, hjust = 0))
...
```{r CK121_CD24_wilcox_heatmap_final, fig.width=9, fig.height=7, fig.path='./output/CK121_CD24/figs/', dev = c('png','pdf')}
...
```{r CK121_CD24_specScore_heatmap_final, fig.width=9, fig.height=7, fig.path='./output/CK121_CD24/figs/', dev = c('png','pdf')}
