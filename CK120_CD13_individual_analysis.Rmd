---
title: "Single-cell analysis on CD13+ sorted cells"
...
## Cell assignment
We tooked a look at the markers from the different clusters from previous section.
...
is the PTPRC gene expression. None clusters are highlighted.

```{r PTPRC,fig.width=4, fig.height=4}
FeaturePlot(SeuratObject, reduction = "umap", features = "PTPRC", label = TRUE)
```

Similarly, we identify potential *endothelial cells* using PECAM1. Highlighting that
the cluster 6 are endothelial cells.
...
Finally we highlight the expression of SLC5A2 and SLC22A6, which is a marker of Proximal
tubule cells (S1 and S2, respectively)

```{r PT,fig.width=10, fig.height=4}
FeaturePlot(SeuratObject, reduction = "umap", features = c("SLC5A2","SLC22A6"), label = TRUE)

ren_id[as.character(0:4)] <- paste0("PT_S1","_",as.character(0:4))
ren_id["5"] <- paste0("PT_S2","_","5")
```
...
| Cluster | rational |
|:---:|:---:|
| Cluster 0 | proximal tubule ( 20% SLC5a2, S1) |
| Cluster 1 | proximal tubule ( 23% SLC5a2, S1) |
| Cluster 2 | proximal tubule ( 21% SLC5a2, S1); STCs? VIM? |
| Cluster 3 | proximal tubule ( 11% SLC5a2, S1) |
| Cluster 4 | proximal tubule ( 13% SLC5a2, S1) |
| Cluster 5 | proximal tubule  no SLC5a2, 37% SLC 22a6, S2? |
...
In conclusion, the new clustering outcome lead to the same as before, but one
cluster was split into two. We rename them accordingly.
...
We checked markers for different PTs in the kidney nephron:
```{r adhoc_dotplot}
DotPlot(SeuratObject, features=c("SLC5A2","SLC22A6","AGT"))
```

Finally we decided the cell type for each cluster - all of them PT, but from different
parts from the nephron based on the dotplot above:
...
```{r CK120_CD13_context_dotplot_final, fig.width=7, fig.height=12, fig.path='./output/CK120_CD13/figs/', dev = c('png','pdf')} 
context_genes <- getGmt("./data/Prior/Kramann_context.gmt")
print(DotPlot(SeuratObject, features = unlist(geneIds(context_genes)), dot.scale = 14) + 
        coord_flip() + scale_y_discrete(position = "right") +
        theme(axis.text.x = element_text(angle = 45, hjust = 0))
...
```{r CK120_CD13_wilcox_heatmap_final, fig.width=9, fig.height=7, fig.path='./output/CK120_CD13/figs/', dev = c('png','pdf')}
...
```{r CK120_CD13_specScore_heatmap_final, fig.width=9, fig.height=7, fig.path='./output/CK120_CD13/figs/', dev = c('png','pdf')}
